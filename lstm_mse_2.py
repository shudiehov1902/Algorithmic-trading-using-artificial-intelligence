"""LSTM (MSE) baseline + portfolio selection with turnover-aware validation.

Drop-in replacement for mlp_mse_2.py with the SAME CLI and evaluation pipeline.
We keep the class name "MLP" but implement it as an LSTM regressor to minimize changes.
Uses args.hidden as hidden_dim and args.depth as num_layers.
Shapes match the original: output (B,1), target (B,1).
"""
