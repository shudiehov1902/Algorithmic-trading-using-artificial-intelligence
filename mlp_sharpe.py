...
def _try_load(path: str) -> Optional[np.ndarray]:
...
def load_split_arrays(data_dir: str, split: str, prefer_mlp: bool = True):
    ...
    dates = _try_load(os.path.join(data_dir, f"{split}_dates.npy"))
    # optional: tickers strings
    tickers = _try_load(os.path.join(data_dir, f"{split}_tickers.npy"))
...
def train_one_epoch_sharpe(
    ...
    # also collect mse regularization (to keep predictions sane)
