...
class SeqDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray, date_id: np.ndarray, ticker_id: np.ndarray):
        ...
        self.d = torch.from_numpy(date_id).long()
        self.t = torch.from_numpy(ticker_id).long()
...
def main():
    ...
    ap.add_argument("--save_path", type=str, default="data/stock_mixer_sortino.pt")
