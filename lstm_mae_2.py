"""LSTM (MAE) baseline + portfolio selection with turnover-aware validation.

This is a drop-in replacement for mlp_mae_2.py with the SAME CLI and evaluation pipeline.
We reuse args.hidden as LSTM hidden_dim and args.depth as num_layers to keep the MLP-style interface.
Shapes are kept consistent with the original script: model outputs (B,1), targets are (B,1).
"""
...
class MLP(nn.Module):
    """LSTM regressor (keeps the class name 'MLP' so the rest of the script stays unchanged).

    Interprets the feature vector as a 1D sequence:
      x: (B, D) -> (B, D, 1), seq_len=D, input_size=1
    Returns:
      (B, 1) to match TabularDataset target shape.
    """
    def __init__(self, input_dim: int, hidden: int = 64, depth: int = 2, dropout: float = 0.1):
        super().__init__()
        self.lstm = nn.LSTM(
            input_size=1,
            hidden_size=hidden,
