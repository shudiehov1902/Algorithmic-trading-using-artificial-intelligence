...
COST_BPS = 10.0                               # 10 bps turnover-cost
...
class DailyCache:
    ...
    rets_sorted: List[np.ndarray]       # aligned returns
...
def backtest_longonly_buffer_cost(
    ...
    for i, d in enumerate(cache.dates):
        ...
        # gross return: mean of returns of held tickers today
    ...
    return gross_s, net_s, to_s


# =====================================================
# 8) Training + per-epoch grid-search on VAL (NET alpha IR, robust H1/H2)
# =====================================================
n_epochs = 50
...
for epoch in range(1, n_epochs + 1):
    ...
    # ---- build VAL cache for this epoch (pred changes each epoch)
    ...
    # ---- grid-search trading params on VAL
    best_score_epoch = -float("inf")
    best_params_epoch = None
    ...
    # ---- update global best (epoch + params)
...
print("Cost config:", f"COST_BPS={COST_BPS}, CHARGE_ENTRY_COST={CHARGE_ENTRY_COST}")
...
# =====================================================
# 10) TEST backtest with selected params + comparisons
# =====================================================
...
# comparisons: fixed (K=5,40,50) with same reb/buf, just to see sensitivity
...
print("Selected params (K,reb,buf):", best_params)
print("Market Sharpe:", sharpe_annual_series(mkt))
print("Selected GROSS Sharpe:", sharpe_annual_series(gross_sel))
print("Selected NET   Sharpe:", sharpe_annual_series(net_sel))
print("Selected GROSS Cum:", cumulative_return(gross_sel))
print("Selected NET   Cum:", cumulative_return(net_sel))
print("Selected avg turnover:", float(to_sel.mean()))
print("Selected Alpha IR (NET):", info_ratio_annual(net_sel, mkt))

print("\n=== TEST COMPARISONS (same reb/buf, different K) ===")
print("K=5   NET Sharpe:", sharpe_annual_series(net_5), "NET Cum:", cumulative_return(net_5), "Alpha IR:", info_ratio_annual(net_5, mkt), "avg TO:", float(to_5.mean()))
print("K=40  NET Sharpe:", sharpe_annual_series(net_40), "NET Cum:", cumulative_return(net_40), "Alpha IR:", info_ratio_annual(net_40, mkt), "avg TO:", float(to_40.mean()))
print("K=50  NET Sharpe:", sharpe_annual_series(net_50), "NET Cum:", cumulative_return(net_50), "Alpha IR:", info_ratio_annual(net_50, mkt), "avg TO:", float(to_50.mean()))
пше